...
static dword swUartReceiveCallBack(UNUSED_FCT_P byte id, dword value)
{
    ...
    if(pSM->state==swURIdle) /* should never happen */
        return(0);
    if(pCfg->bTripleScan)
    { /* noise canceler: scans three times 1/4 bit then waits for 1/2 bit */
    ...
    case swURSBit:
        ...
        if(++pSM->c.bitPos>=pCfg->nbBits)
        {   /* reached required number of bits then stop bit if no parity or parity check */
...
word swUartGetChar(byte swUartRxId)
{
    ...
    if(pSM->rxFifoReadIndex==pSM->rxFifoWriteIndex)
    {   /* FIFO dried out */
        pSM->rxFifoReadIndex=pSM->rxFifoWriteIndex=0;
        pSM->rxFifoEmpty=TRUE;
    }
